...
let sumGrade = anketa(QUESTIONS)

alert(`Вы набрали ${sumGrade} баллов`)
 ...
 if (item.type === 'prompt') {
   userAnswer = prompt(item.question) 
    if (userAnswer === item.answer) {
      answersGrades.push(10)
        } 
} else { 
  userAnswer = confirm(item.question)
    if (userAnswer === item.answer) {
      answersGrades.push(10)
        }  
    }
}
const result = answersGrades.reduce((accumulator, current) => {
